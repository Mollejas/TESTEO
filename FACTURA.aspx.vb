...
Public Class FACTURA
    ...
    Private Sub GenerarPDF(xmlPath As String, pdfPath As String)
        ...
        ' 2) Asegurar carpeta del PDF
        Dim pdfDir As String = Path.GetDirectoryName(pdfPath)
        If Not Directory.Exists(pdfDir) Then
            Directory.CreateDirectory(pdfDir)
        End If

        ' ⚠️ ESTA VERSIÓN NI SIQUIERA LEE EL XML
        ' Solo genera un PDF de prueba para descartar problemas con iTextSharp / FileStream.

        Using fs As New FileStream(pdfPath, FileMode.Create, FileAccess.Write, FileShare.None)
            ' Aquí creamos SIEMPRE una nueva instancia de Document
            Dim doc As New iTextSharp.text.Document(iTextSharp.text.PageSize.LETTER, 36, 36, 36, 36)

            ' 👉 Si aquí diera NullReference, es algo externo (ensamblado / binarios / referencia)
            Dim writer As iTextSharp.text.pdf.PdfWriter =
                iTextSharp.text.pdf.PdfWriter.GetInstance(doc, fs)

            doc.Open()

            Dim tituloFont As New iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 18, iTextSharp.text.Font.BOLD)
            Dim normalFont As New iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 10, iTextSharp.text.Font.NORMAL)

            doc.Add(New iTextSharp.text.Paragraph("PRUEBA PDF", tituloFont))
            doc.Add(New iTextSharp.text.Paragraph("Si ves este PDF, iTextSharp funciona correctamente.", normalFont))
            doc.Add(New iTextSharp.text.Paragraph("Ruta del XML usado (no leído en esta prueba):", normalFont))
            doc.Add(New iTextSharp.text.Paragraph(xmlPath, normalFont))
